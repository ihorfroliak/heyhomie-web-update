...
import CancelledServiceCard from './CancelledServiceCard';
...
const TabSelectionButton = styled.button`
    ...
    ${props =>
        props.selected
            ? css`
                  border-bottom: 1px solid #14133a;
                  color: #14133a;
              `
            : css`
                  color: #727189;
...
`;
const ActiveServicesWrapper = styled.div`
    ${props =>
        props.isLoading
            ? css`
                  opacity: 0.5;
...
const CancelledServicesWrapper = styled.div`
    ${props =>
        props.isLoading
            ? css`
                  opacity: 0.5;
...
// Utils
function serviceStatusColorSwitch(status) {
    switch (status) {
...
const ServicesComponent = ({
    ...
    activeServices,
    setActiveServices,
    activeServicesVisibleNumber,
    setActiveServicesVisibleNumber,
    visibleActiveServices,
    setVisibleActiveServices,
    activeServicesLoading,
    setActiveServicesLoading,
    cancelledServices,
    setCancelledServices,
    cancelledServicesVisibleNumber,
    setCancelledServicesVisibleNumber,
    visibleCancelledServices,
    setVisibleCancelledServices,
    cancelledServicesLoading,
    setCancelledServicesLoading,
    ...
    function parseDateTime(date_timeObject) {
        ...
        const string = `${month} ${day}, ${year} ${t(`utils.time_at`)} ${time}:00`;
        return string;
    }
    ...
    useEffect(() => {
        async function fetchServices() {
            ...
            try {
                const data = await listServices(user.headers, user.x_token_user, 'active');

                const activeServicesWorking = data.services.filter(s => s.status === 'active' || s.status === 'freezed');

                const cancelledServicesWorking = data.services.filter(s => s.status === 'canceled');

                setActiveServices(activeServices => [...activeServicesWorking]);

                setCancelledServices(cancelledServices => [...cancelledServicesWorking]);
    ...
    useEffect(() => {
        let workingArray = [...activeServices];

        workingArray = workingArray.filter((m, i) => i + 1 <= activeServicesVisibleNumber);

        setVisibleActiveServices(visibleActiveServices => [...workingArray]);
    }, [activeServicesVisibleNumber, activeServices]);

    useEffect(() => {
        let workingArray = [...cancelledServices];

        workingArray = workingArray.filter((m, i) => i + 1 <= cancelledServicesVisibleNumber);

        setVisibleCancelledServices(cancelledActiveServices => [...workingArray]);
    }, [cancelledServicesVisibleNumber, cancelledServices]);

    return (
        <ServicesContainer>
            {!initialFetchLoading ? (
                activeServices.length === 0 && cancelledServices.length === 0 ? (
                    <EmptyContainer>
                        <div style={{ width: '100%' }}>
                            <svg
                                style={{
                            ...
                            >
                                ...
                                <path
                                    ...
                                    fill='#14133A'
                                />
                                <path fillRule='evenodd' clipRule='evenodd' d='M167.916 90.088H19.9014V84.5729H167.916V90.088Z' fill='#14133A' />
                            </svg>
                        </div>
                        <PrimaryButtonFull
                            style={{
                    ...
                    <ServicesWrapper>
                        <TabSelection>
                            <TabSelectionButton
                                onClick={() => {
                                    setCurrentVisible('active');
                                }}
                                disabled={currentVisible === 'active'}
                                selected={currentVisible === 'active'}
                            >
                                {t(`active_tabBtn`)}
                            </TabSelectionButton>
                            <TabSelectionButton
                                onClick={() => {
                                    setCurrentVisible('cancelled');
                                }}
                                disabled={currentVisible === 'cancelled'}
                                selected={currentVisible === 'cancelled'}
                            >
                                {t(`cancelled_tabBtn`)}
                            </TabSelectionButton>
                        </TabSelection>
                        {currentVisible === 'active' ? (
                            <ActiveServicesWrapper isLoading={activeServicesLoading || cancelledServicesLoading}>
                                {visibleActiveServices.length > 0 &&
                                    visibleActiveServices.map(service => (
                                        <ActiveServiceCard
                                            key={service.id}
                                            service={service}
                                            parseDateTime={parseDateTime}
                                            serviceStatusColorSwitch={serviceStatusColorSwitch}
                                ...
                                <SeeMoreButton
                                    onClick={() => {
                                        setActiveServicesVisibleNumber(activeServicesVisibleNumber => activeServicesVisibleNumber + 4);
                                    }}
                                    disabled={activeServicesVisibleNumber >= activeServices.length}
                                >
                                    {t(`seeMore_btn`)}
                                </SeeMoreButton>
                            </ActiveServicesWrapper>
                        ) : (
                            <CancelledServicesWrapper isLoading={activeServicesLoading || cancelledServicesLoading}>
                                {visibleCancelledServices.length > 0 &&
                                    visibleCancelledServices.map(service => (
                                ...
                                <SeeMoreButton
                                    onClick={() => {
                                        setCancelledServicesVisibleNumber(cancelledServicesVisibleNumber => cancelledServicesVisibleNumber + 4);
                                    }}
                                    disabled={cancelledServicesVisibleNumber >= cancelledServices.length}
                                >
                                    {t(`seeMore_btn`)}
                                </SeeMoreButton>
                            </CancelledServicesWrapper>
                        )}
                        <div
                            style={{
                                marginTop: '64px',
